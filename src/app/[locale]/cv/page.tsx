          ...
          <aside className="px-5 py-6 md:px-10 md:py-10 bg-slate-100/50 dark:bg-muted border-r-0 md:border-r border-border dark:border-border print:float-left print:w-[65mm] print:bg-slate-100/50 print:border-r print:border-border print:px-[6mm] print:py-[8mm]"
            ...
            {techSkills.length > 0 && (
              <section className="mb-6 md:mb-8 print:mb-4">
                <h2 className="text-base md:text-sm font-bold uppercase tracking-[0.14em] text-primary dark:text-foreground mb-3 pb-2 border-b border-secondary/40">{t("cv.skills")}</h2>
            ...
            {languages.length > 0 && (
              <section className="mb-6 md:mb-8 print:mb-4">
                <h2 className="text-base md:text-sm font-bold uppercase tracking-[0.14em] text-primary dark:text-foreground mb-3 pb-2 border-b border-secondary/40">
                  {locale === "fr" ? "Langues" : locale === "en" ? "Languages" : "اللغات"}
                </h2>
                <div className="flex flex-wrap gap-1.5">
                  {languages.map((lang: any) => (
                    <span key={lang.id} className="inline-block text-sm md:text-xs px-2 py-0.5 rounded-sm border border-secondary/40/30 dark:border-foreground/30 text-primary dark:text-foreground font-medium">
            ...
            {education.length > 0 && (
              <section className="mb-6 md:mb-8 print:mb-4">
                <h2 className="text-base md:text-sm font-bold uppercase tracking-[0.14em] text-primary dark:text-foreground mb-3 pb-2 border-b border-secondary/40">{t("cv.education")}</h2>
              ...
              <section>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {interests.map((interest: any) => (
                    <span key={interest.id} className="inline-block text-sm md:text-xs px-2 py-0.5 rounded-sm bg-secondary/10 dark:bg-secondary/15 text-primary dark:text-foreground font-medium"
                      style={{ printColorAdjust: "exact", WebkitPrintColorAdjust: "exact" }}>
                      {interest.name}
                    </span>
                  ))}
          ...
          <main className="px-5 py-6 md:px-10 md:py-10 print:overflow-hidden print:px-[6mm] print:py-[8mm]">
            ...
            {getBio() && (
              <section className="mb-6 md:mb-8 break-inside-avoid">
                <h2 className="text-base md:text-sm font-bold uppercase tracking-[0.14em] text-primary dark:text-foreground mb-3 pb-2 border-b border-secondary/40">{t("cv.profile")}</h2>
            ...
            {mainExperiences.length > 0 && (
              <section className="mb-6 md:mb-8 print:mb-4">
                <h2 className="text-base md:text-sm font-bold uppercase tracking-[0.14em] text-primary dark:text-foreground mb-4 pb-2 border-b border-secondary/40">{t("cv.experience")}</h2>
            ...
            {featuredProjects.length > 0 && (
              <section>
                <h2 className="text-base md:text-sm font-bold uppercase tracking-[0.14em] text-primary dark:text-foreground mb-4 pb-2 border-b border-secondary/40">{t("projects.title")}</h2>
                <div className="space-y-4 print:space-y-3">
                    ...
                    <div key={project.id}>
                      ...
                      <div className="flex flex-wrap gap-1.5 mt-2">
                        {project.technologies.slice(0, 6).map((tech: string) => (
                          <span key={tech} className="text-sm md:text-xs px-1.5 py-0.5 rounded-sm bg-secondary/10 dark:bg-secondary/10 text-primary dark:text-foreground font-medium">{tech}</span>
                        ))}
                      </div>
                    </div>
                  ))}
                        ...
                        <ul className="mt-1 space-y-0.5">
                          {toBullets(exp.description).map((b: string, i: number) => (
                            <li key={i} className="text-[9pt] text-primary/80 leading-snug flex items-start gap-1.5">
                              <span className="mt-1 shrink-0 w-1 h-1 rounded-full bg-secondary" />
                              <span>{b}</span>
                            </li>
                          ))}
                        </ul>
